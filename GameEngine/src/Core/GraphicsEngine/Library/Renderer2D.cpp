...
namespace GraphicsEngine
{
	...
	{
		data.vertexArrayBuffer = GraphicsEngine::GetLibrary()->CreateVertexArrayBuffer();
		data.vertexArrayBuffer->Init();

		data.vertexDynamicBuffer = GraphicsEngine::GetLibrary()->CreateVertexDynamicBuffer(data.batchVerticesSize * sizeof(Quad));
		...
		std::shared_ptr<IndexStaticBuffer> indexBuffer = GraphicsEngine::GetLibrary()->CreateIndexStaticBuffer(quadIndices, data.batchIndicesSize);
		indexBuffer->Init();
		delete[] quadIndices;
	...
	void Renderer2D::InitAssets() noexcept
	{
		...
		data.whiteTexture = GraphicsEngine::GetLibrary()->CreateTexture(1, 1);
		...
		data.shaderProgram = GraphicsEngine::GetLibrary()->CreateShaderProgram("assets/shaders/default_2d_shader");
